(function ($) {
  ...
  if ($(".sticky-header__content").length) {
    ...
    // Create a robust structure for flex styling
    mobileNavContainer.innerHTML = `
        <div class="sticky-header__logo">${logoContent}</div>
        <div class="sticky-header__menu">${navContent}</div>
    `;
  }
  ...
  $(window).on("scroll", function () {
    if ($(".stricked-menu").length) {
      ...
      var scrollDifference = Math.abs(currentScroll - window.lastScrollTop);
      var scrollThreshold = 10; // Minimum scroll amount to trigger change

      if (currentScroll > headerScrollPos) {
      ...
      window.lastScrollTop = currentScroll <= 0 ? 0 : currentScroll; // For Mobile or negative scrolling
    }
